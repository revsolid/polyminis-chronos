...
class Chronos:
    ...
    class ChronosDBHandler:
        ...
        def __init__(self, config):
            self.url = config.get('SimUrl', 'http://localhost:80')
...
#        Repeat as needed (Steps 2 & 3 could repeat if more than one Epoch is supposed to be advanced)
        ...
        if len(load) == 2: 
            ...
            payload = { "MasterTranslationTable": master_tt,
                            "EpochNum": e_num,
                            "Species": species }
            payload["Epoch"] = self.db_handler.get_planet_data(e_num)
        else:
            planet_id = -1
            e_num = 1 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--db_url',  metavar='DBUrl',  type=str, help='URL of the Persistence Server', default='http://localhost:80')
